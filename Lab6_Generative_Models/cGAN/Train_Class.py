...
def main():
    ...
    print(device)
    savepath = os.path.join('cGAN_continue_Train_1700')
    ...
    # print(list(json_labels.keys()))
    # print(json_labels.get(list(json_labels.keys())[0], []))
    # a
    root = './Lab6_dataset'
    ...
    ## Generator
    checkpoint_gendis = \
    torch.load('./saved_models_cGAN_3/Model-ep1900.pth')
    Gen = Generator()
    Gen.load_state_dict(checkpoint_gendis['Gen_state_dict'])
    Gen = Gen.to(device = device)
    # Discriminator
    checkpoint_gendis2 = \
    torch.load('./saved_models_cGAN_3/Model-ep1000.pth')
    Dis = Discriminator()
    Dis.load_state_dict(checkpoint_gendis2['Dis_state_dict'])
    ...
    # optimizerGen.load_state_dict(checkpoint_gendis['Gen_optimizer'])
    ...
    test_json_labels = load_json_labels('/mnt/left/home/2023/angelina/DLCourse/Lab6/Lab6_code/test.json')

    labels = test_json_labels[1]
    # print(label_mapping)
    test_LAB = []
    for i in range(len(test_json_labels)):
    # 將多標籤轉換為多熱編碼
        label_vector = np.zeros(len(label_mapping), dtype=np.float32)
        label_vector_class = np.zeros(len(label_mapping), dtype=np.float32)
        labels = test_json_labels[i]
        for label in labels:
            label_index = label_mapping.get(label)
            # print(label_index)
        ...
        test_LAB.append(label_vector)
    test_LAB = torch.stack(test_LAB)
    # print(LAB)
    ...
    for epoch in range(num_epochs):
        ...
        for a, (real_images, class_labels, class_labels_class) in enumerate(tqdm(train_dataloader, desc=f'Epoch {epoch}/{num_epochs}')):
            ...
            Generator_loss = criterion(output, true_label_update) + lamda * class_loss_fake
        ...
        np.savez(os.path.join('GANC_training_process.npz'), **His_Train_Process)
